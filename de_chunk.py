...
for audio_name in audio_file_names:
    ...
    while lower_limit < len(curr_audio_file[0]):
        if upper_limit<len(curr_audio_file[0]):
            sf.write(clip_audio_datasets+"channel1_"+clip_audio_file_name+str(cnt)+'_'+str(micro_cnt)+'.wav', curr_audio_file[0][lower_limit:upper_limit], current_sample_rate)
            sf.write(clip_audio_datasets+"channel2_"+clip_audio_file_name+str(cnt)+'_'+str(micro_cnt)+'.wav', curr_audio_file[1][lower_limit:upper_limit], current_sample_rate)
        else:
            sf.write(clip_audio_datasets+"channel1_"+clip_audio_file_name+str(cnt)+'_'+str(micro_cnt)+'.wav', curr_audio_file[0][lower_limit:], current_sample_rate)
            sf.write(clip_audio_datasets+"channel2_"+clip_audio_file_name+str(cnt)+'_'+str(micro_cnt)+'.wav', curr_audio_file[1][lower_limit:], current_sample_rate)
